  ...
  static generateStatementHtml(userData, memberNumber) {
    return `
      <div class="statement">
        ...
        <p>В случае моего неучастия** в жизни и развитии Клуба в течение 3 месяцев, прошу считать меня выбывшим из Кооператива по собственному желанию. Взнос в 1 рубль прошу вернуть путем пополнения мобильной связи телефона, указанного в заявлении. (**Участие – это внесение паевых взносов для удовлетворения своих потребностей).</p>
        ...
        <p>2. Индивидуальный № пайщика (для идентификации взноса) ${memberNumber || 'Не присвоен'}</p>
        <p>3. Год рождения: ${userData.yeardate || 'Не указан'}</p>
        <p>4. Электронная почта: ${userData.email || 'Не указан'}</p>
        <p>5. Телефон: ${userData.phone || 'Не указан'}</p>
        <p>6. Место жительства: ${userData.city || 'Не указан'}</p>
        
        <p>Информация, внесенная в Заявление, предназначена только для внутреннего использования. Кооператив гарантирует, что персональные данные не распространяются, а также не предоставляются третьим лицам без согласия субъекта персональных данных и используются кооперативом исключительно для заключения договоров с субъектом персональных данных (№152-ФЗ).</p>
        
        <p>Своей подписью я подтверждаю достоверность переданных данных (самоидентификацию) и согласие с условиями участия в качестве члена Клуба НПК «В ДУХЕ ВРЕМЕНИ».</p>
      </div>
    `;
  }
